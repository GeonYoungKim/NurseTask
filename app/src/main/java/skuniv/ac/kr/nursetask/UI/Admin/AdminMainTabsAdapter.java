...
public class AdminMainTabsAdapter extends FragmentStatePagerAdapter implements TabHost.OnTabChangeListener, ViewPager.OnPageChangeListener{
    ...
    public AdminMainTabsAdapter(Context context, TabHost tabHost, ViewPager viewPager ) {
        ...
        // init
        int count = AdminMainTabsConfig.COUNT_TABS();
        for( int i = 0; i < count; i++ ) {
            AdminMainTabsConfig.TabInfo tabInfo = AdminMainTabsConfig.TABINFO( i );
    ...
    @Override
    public int getCount() {
        return AdminMainTabsConfig.COUNT_TABS();
    }

    @Override
    public Fragment getItem(int index ) {
        AdminMainTabsConfig.TabInfo tabInfo = AdminMainTabsConfig.TABINFO( index );
    ...
    public void onTabChanged( String tabId ) {
        AdminMainTabsConfig.TabInfo tabInfo;

        // previous tab
        if( currentTabView != null && currentIndex >= 0 ) {
            tabInfo = AdminMainTabsConfig.TABINFO( currentIndex );
            if( tabInfo.drawableNormal != 0  && tabInfo.drawableSelected != 0 ) {
